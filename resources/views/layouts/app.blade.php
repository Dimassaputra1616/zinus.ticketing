...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        @php
            ...
            $pageTitle = match (true) {
                ...
                request()->routeIs('admin.assets.automation-console') => 'Automation Console',
    ...
    >
	                    ...
	                    'children' => [
	                        ...
	                        [
	                            ...
	                            'children' => [
	                                ...
	                                ['type' => 'link', 'label' => 'Automation Console', 'route' => 'admin.assets.automation-console', 'visible' => $isAdmin],
        ...
        <main class="min-h-screen flex min-w-full flex-col relative z-10 w-full max-w-full lg:min-w-0 lg:flex-1" style="padding-top: env(safe-area-inset-top)">
                    ...
                    @if ($user)
                        <div class="relative">
                            <button 
                                ...
                                class="flex items-center gap-1.5 rounded-full bg-white/10 px-2 py-1 transition-colors hover:bg-white/20"
                            >
                                <span class="h-7 w-7 rounded-full bg-emerald-600 text-white text-[11px] flex items-center justify-center font-bold ring-1 ring-emerald-400/30">
                                    {{ mb_strtoupper(mb_substr($user->name, 0, 1)) }}
                                </span>
                                <svg class="h-3 w-3 text-white/60 transition-transform" :class="mobileProfileOpen && 'rotate-180'" viewBox="0 0 20 20" fill="currentColor">
                            ...
                            <div
                                ...
                                class="absolute right-0 mt-2 w-56 origin-top-right rounded-2xl border border-white/10 bg-[#0d1f1a]/95 p-3 shadow-2xl shadow-black/50 backdrop-blur-xl"
                                style="display:none"
                            >
                                {{-- User Info --}}
                                <div class="mb-3 flex items-center gap-3 rounded-xl bg-white/5 px-3 py-2.5">
                                    <span class="h-9 w-9 flex-shrink-0 rounded-full bg-emerald-600 text-white flex items-center justify-center text-sm font-bold ring-2 ring-emerald-400/30">
                                        {{ mb_strtoupper(mb_substr($user->name, 0, 1)) }}
                                    </span>
                                    <div class="min-w-0">
                                        <p class="text-sm font-semibold text-white truncate">{{ $user->name }}</p>
                                        <p class="text-[10px] text-emerald-300/60 truncate">{{ $user->email }}</p>
                                    </div>
                                </div>

                                {{-- Language Switcher --}}
                                <div class="mb-2">
                                    <p class="mb-1.5 text-[9px] font-bold uppercase tracking-wider text-white/30">Language</p>
                                    <div class="flex gap-1">
                                        @foreach (['en' => 'EN', 'id' => 'ID', 'ko' => 'KO'] as $locale => $label)
                                            <a 
                                                href="{{ route('lang.switch', $locale) }}"
                                                class="flex-1 rounded-lg py-1.5 text-center text-[10px] font-bold transition-all {{ app()->getLocale() === $locale ? 'bg-emerald-500/30 text-emerald-300 ring-1 ring-emerald-400/30' : 'bg-white/5 text-white/50 hover:bg-white/10 hover:text-white/80' }}"
                                            >{{ $label }}</a>
                                        @endforeach
                                    </div>
                                </div>
                                ...
                                <form method="POST" action="{{ route('logout') }}">
                                    @csrf
                                    <button type="submit" class="flex w-full items-center gap-2 rounded-xl bg-rose-500/10 px-3 py-2 text-xs font-semibold text-rose-400 transition-colors hover:bg-rose-500/20">
                ...
                {{-- Mobile top bar removed to save space, redundant with PWA & title headers --}}
                ...
                @if (request()->routeIs('dashboard') || request()->routeIs('tickets.mine') || request()->routeIs('tickets.index') || request()->routeIs('tickets.show') || request()->routeIs('user.tickets.show') || request()->routeIs('users.*') || request()->routeIs('loans.*') || request()->routeIs('assets.*') || request()->routeIs('admin.assets.*') || request()->routeIs('admin.conversations.*') || request()->routeIs('remote-system.*') || request()->routeIs('admin.settings.index') || request()->routeIs('tutorials.*') || request()->routeIs('admin.tutorials.*') || request()->routeIs('profile.edit'))
                    @php
                        ...
                        $topbarDescription = match (true) {
                            ...
                            request()->routeIs('admin.assets.automation-console') => 'Jalankan automation asset yang sudah disiapkan dari panel command terkontrol.',
                ...
                @php
                    $wideWorkspace = request()->routeIs(
                        ...
                        'admin.assets.automation-console',
                        'admin.assets.bast.*',
        ...
        <footer class="mt-auto bg-white/90 border-t border-slate-200 py-4 hidden lg:block">
            <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between text-xs text-slate-500">
                ...
                <style>
                    ...
                    @keyframes scanSweep {
                        0%, 100% { opacity: .25; transform: translateY(-11px); }
                        45%, 55% { opacity: .9; }
                    ...
                    .mobile-nav-scan-core {
                        ...
                        justify-content: center;
                        overflow: hidden;
                        border: 1px solid rgba(167, 243, 208, .48);
                        border-radius: 1.35rem;
                        background: linear-gradient(155deg, #34d399 0%, #10b981 48%, #0f766e 100%);
                        color: #ecfdf5;
                        box-shadow: 0 16px 34px rgba(16, 185, 129, .34), 0 8px 18px rgba(2, 6, 23, .34), inset 0 1px 0 rgba(255, 255, 255, .36);
                        transition: transform .2s ease, box-shadow .2s ease, background .2s ease;
                    }
                    .mobile-nav-scan-core::before {
                        content: "";
                        position: absolute;
                        inset: -28%;
                        z-index: 0;
                        background: radial-gradient(circle at 35% 20%, rgba(255, 255, 255, .44), transparent 34%), radial-gradient(circle at 78% 82%, rgba(6, 78, 59, .28), transparent 38%);
                    }
                    .mobile-nav-scan-core::after {
                        content: "";
                        position: absolute;
                        left: .9rem;
                        right: .9rem;
                        ...
                        box-shadow: 0 0 12px rgba(167, 243, 208, .95);
                        animation: scanSweep 2.4s ease-in-out infinite;
                    }
                    .mobile-nav-scan:active .mobile-nav-scan-core { transform: translateY(1px) scale(.97); }
                    .mobile-nav-scan.is-active .mobile-nav-scan-core {
                        background: linear-gradient(155deg, #d1fae5 0%, #34d399 46%, #10b981 100%);
                        color: #064e3b;
                        box-shadow: 0 18px 38px rgba(52, 211, 153, .44), 0 8px 20px rgba(2, 6, 23, .36), inset 0 1px 0 rgba(255, 255, 255, .6);
                    }
                ...
                <div data-pwa-bottom-nav-shell class="pointer-events-none fixed inset-x-0 bottom-0 z-50 px-3 lg:hidden" style="top: auto; bottom: var(--pwa-bottom-offset, 0px); padding-bottom: max(env(safe-area-inset-bottom), 10px)">
                    <nav class="pwa-bottom-nav pointer-events-auto relative mx-auto max-w-md items-end gap-1.5 rounded-[1.65rem] border border-white/10 px-2 pb-1.5 pt-2 shadow-[0_18px_48px_rgba(2,6,23,0.34)] backdrop-blur-2xl" style="grid-template-columns: repeat({{ $isAdmin ? 5 : 4 }}, minmax(0, 1fr)) !important; background: linear-gradient(180deg, rgba(10, 44, 35, .94), rgba(4, 24, 19, .96));">
                        ...
                        @php
                            ...
                            $mobileSlots = $isAdmin
                                ? [
                                    ['type' => 'item', 'item' => $mobileNavByRoute->get('dashboard')],
                                    ['type' => 'item', 'item' => $mobileNavByRoute->get('tickets.index')],
                                    ['type' => 'scan'],
                                    ['type' => 'item', 'item' => $mobileNavByRoute->get('tutorials.index')],
                                    ['type' => 'item', 'item' => $mobileNavByRoute->get('assets.index')],
                                ]
                                : [
                                    ['type' => 'item', 'item' => $mobileNavByRoute->get('dashboard')],
                                    ['type' => 'item', 'item' => $mobileNavByRoute->get('tickets.mine')],
                                    ['type' => 'item', 'item' => $mobileNavByRoute->get('loans.index')],
                                    ['type' => 'item', 'item' => $mobileNavByRoute->get('tutorials.index')],
                                ];
                        ...
                        @foreach ($mobileSlots as $slot)
                            ...
                            @if (($slot['type'] ?? null) === 'scan')
                                <a
                                    href="{{ route('admin.assets.scan') }}"
                                    class="mobile-nav-scan {{ $isScanActive ? 'is-active text-emerald-100' : 'text-emerald-50/80 hover:text-emerald-50' }} relative -mt-5 flex h-[76px] min-w-0 flex-col items-center justify-end gap-1 text-center"
                                    aria-label="Scan asset"
                                >
                                    <span class="mobile-nav-scan-core">
                                        <svg class="relative z-10 h-6 w-6" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.95" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
                                    ...
                                    <span class="max-w-full truncate text-[10px] font-black tracking-normal">Scan</span>
                                    @if ($isScanActive)
                                        <span class="absolute bottom-0.5 left-1/2 h-1 w-5 -translate-x-1/2 rounded-full bg-emerald-300 shadow-[0_0_10px_rgba(110,231,183,.9)]"></span>
                                    @endif
                                </a>
                            @elseif ($item)
                                @php
        ...
        <script>
            document.addEventListener('DOMContentLoaded', () => {
                const pwaBottomNavShell = document.querySelector('[data-pwa-bottom-nav-shell]');
                const syncPwaBottomNavOffset = () => {
                    if (!pwaBottomNavShell) return;

                    const visualViewport = window.visualViewport;
                    if (!visualViewport) {
                        pwaBottomNavShell.style.setProperty('--pwa-bottom-offset', '0px');
                        return;
                    }
                ...
                window.addEventListener('resize', syncPwaBottomNavOffset, { passive: true });
                window.addEventListener('orientationchange', syncPwaBottomNavOffset, { passive: true });
                window.visualViewport?.addEventListener('resize', syncPwaBottomNavOffset, { passive: true });
                window.visualViewport?.addEventListener('scroll', syncPwaBottomNavOffset, { passive: true });
