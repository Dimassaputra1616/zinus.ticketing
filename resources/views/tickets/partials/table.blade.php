                ...
                <article class="rounded-[20px] border border-slate-100 bg-white/95 p-4 shadow-[0_10px_36px_rgba(0,0,0,0.06)] animate-fade-up transition duration-200 hover:-translate-y-[2px] hover:scale-[1.01] hover:shadow-[0_12px_40px_rgba(0,0,0,0.08)] hover:shadow-emerald-100/70">
                    <div class="flex flex-col gap-3">
                        ...
                        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                            ...
                            <form
                                id="status-form-mobile-{{ $ticket->id }}"
                                method="POST"
                            ...
                            >
                                ...
                                <div class="flex flex-col gap-1">
                                    ...
                                    <select
                                        ...
                                        onchange="if (this.value === this.dataset.current) return; const label = this.options[this.selectedIndex] ? this.options[this.selectedIndex].text : this.value; window.dispatchEvent(new CustomEvent('open-confirm-modal', { detail: { title: 'Ubah Status', message: 'Ubah status tiket #' + this.dataset.ticket + ' ke ' + label + '?', onConfirm: () => { if (this.value === 'resolved') { window.dispatchEvent(new CustomEvent('open-resolution', { detail: { ticketId: this.dataset.ticket, formId: 'status-form-mobile-' + this.dataset.ticket } })); return; } this.dataset.current = this.value; if (this.form.requestSubmit) { this.form.requestSubmit(); } else { this.form.submit(); } }, onCancel: () => { this.value = this.dataset.current; } } }));"
                                        ...
                                        <tr class="table-hover-row group transition duration-150 {{ $loop->even ? 'bg-emerald-50/10' : 'bg-white' }} hover:bg-emerald-50/40 hover:shadow-md align-top">
                                            ...
                                            <td class="px-2 py-2 text-right align-top overflow-visible w-[280px] min-w-[280px] whitespace-nowrap">
                                                <div class="flex flex-col items-end gap-2 opacity-80 transition group-hover:opacity-100 md:flex-row md:items-center md:justify-end">
                                                    ...
                                                    <form
                                                        id="status-form-desktop-{{ $ticket->id }}"
                                                        method="POST"
                                                        action="{{ route('tickets.updateStatus', $ticket) }}"
                                                    ...
                                                    >
                                                        ...
                                                        <div class="relative w-full md:w-auto">
                                                            <select
                                                                ...
                                                                onchange="if (this.value === this.dataset.current) return; const label = this.options[this.selectedIndex] ? this.options[this.selectedIndex].text : this.value; window.dispatchEvent(new CustomEvent('open-confirm-modal', { detail: { title: 'Ubah Status', message: 'Ubah status tiket #' + this.dataset.ticket + ' ke ' + label + '?', onConfirm: () => { if (this.value === 'resolved') { window.dispatchEvent(new CustomEvent('open-resolution', { detail: { ticketId: this.dataset.ticket, formId: 'status-form-desktop-' + this.dataset.ticket } })); return; } this.dataset.current = this.value; if (this.form.requestSubmit) { this.form.requestSubmit(); } else { this.form.submit(); } }, onCancel: () => { this.value = this.dataset.current; } } }));"
                                                                aria-label="Ubah status tiket"
                                                                title="Ubah status tiket"
                                                            >
                                                                @foreach ($statuses as $value => $label)
...
<div x-data="{
    ...
    openModal(event) {
        ...
        this.lockScroll();
        this.$nextTick(() => {
            const ta = document.getElementById('global_resolution');
            if (ta) ta.focus();
        });
    },
    submit() {
        if (!this.resolution || this.resolution.length < 10) return;
        ...
        input.value = this.resolution;
        form.appendChild(input);

        const select = form.querySelector('select[name=status]');
        if (select) select.dataset.current = 'resolved';
...
}" x-init="$nextTick(() => document.body.appendChild($el))" @open-resolution.window="openModal" @keydown.escape.window="if(show) cancel()">
    {{-- Resolution Modal --}}
    {{-- Backdrop --}}
    <div
        x-show="show"
    ...
    >
        ...
        >
            ...
            <div class="relative px-7 pt-7 pb-1">
                <button @click="cancel" class="absolute right-4 top-4 flex h-8 w-8 items-center justify-center rounded-full text-slate-400 transition-all duration-200 hover:bg-slate-100 hover:text-slate-600 hover:rotate-90">
                ...
                <div class="mt-4 inline-flex items-center gap-2 rounded-full border border-emerald-200 bg-emerald-50 px-3 py-1">
                    ...
                    <span class="text-xs font-semibold text-emerald-700" x-text="'Tiket #' + ticketId"></span>
                </div>
            </div>

            {{-- Body --}}
            <div class="px-7 py-5">
                <label for="global_resolution" class="mb-2 flex items-center justify-between">
            ...
            <div class="flex items-center justify-end gap-3 border-t border-slate-100 bg-gradient-to-r from-slate-50 to-emerald-50/30 px-7 py-4">
                <button type="button" @click="cancel" class="rounded-xl border border-slate-200 bg-white px-5 py-2.5 text-sm font-semibold text-slate-600 shadow-sm transition-all duration-200 hover:-translate-y-0.5 hover:border-slate-300 hover:shadow-md active:translate-y-0">
                    {{ __('messages.cancel') }}
                </button>
                <button type="button" @click="submit" :disabled="!resolution || resolution.length < 10" class="group relative overflow-hidden rounded-xl bg-gradient-to-r from-emerald-600 to-teal-600 px-6 py-2.5 text-sm font-semibold text-white shadow-lg shadow-emerald-500/25 transition-all duration-200 hover:-translate-y-0.5 hover:shadow-xl hover:shadow-emerald-500/30 active:translate-y-0 disabled:opacity-40 disabled:shadow-none disabled:hover:translate-y-0">
