...
class AssetScanTest extends TestCase
{
    ...
    public function test_admin_mobile_bottom_nav_uses_fixed_scan_layout(): void
    {
        ...
        $this->actingAs($admin)
            ...
            ->assertSee('safeBottomGap', false)
            ->assertSee('visualBottomGap <= 96', false)
            ->assertSee('position: fixed !important', false)
            ->assertSee('pointer-events-none fixed inset-x-0 bottom-0', false)
            ->assertSee('max-w-md', false)
            ->assertSee('rounded-[1.65rem]', false)
            ->assertSee('linear-gradient(180deg, rgba(10, 44, 35, .94)', false)
            ->assertDontSee('width: 100vw', false)
